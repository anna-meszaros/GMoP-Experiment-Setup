...
def get_crossing_trajectories(Y, T, device='cpu'):
    ...
    for i in range(len(d1_pairs)):
        if torch.any(d1_pairs[i]):
            ...
            assert ~dist[:,triu_indices[0],triu_indices[1],:,:].reshape(-1, n_timesteps, n_timesteps)[i].isnan().any()
    ...
    for i in range(n_batches):
        ...
        # assert ~Y[i,agent_ids_gen[mask.cpu()].max()].isnan().any()
...
def get_hypothetical_path_crossing(X, Y, T, dt, dataset, device='cpu'):
    ...
    if dataset != 'Argoverse' and dataset != 'Interaction':
        ...
        X_t0 = torch.cat([X_t0, heading[..., None]], axis=-1)
    else:
    ...
    # Calculate the interactions based on crossing trajectories
    # Note: The crossing trajectories are calculated based on the hypothetical trajectories, not the original ones
    # Additionally, interaction is determined by checking whether in the case of "no interacition" based on the actual trajectories
    # there is a crossing in the hypothetical trajectories; if so check which agent of the pair reached the hypothetical crossing
    # first in reality


    n_batches = Y_hypothetical.shape[0]
    # Get maximum number of agents
    n_agents = Y_hypothetical.shape[1]

    # Get number of timesteps
    n_timesteps = Y_hypothetical.shape[2]

    dist = calculate_distance_matrix(Y_hypothetical)
    ...
    for i in range(len(d1_pairs)):
        if torch.any(d1_pairs[i]):
            ...
            hyp_cross_point_agent1 = Y_hypothetical[int(np.floor(i/num_agent_pairs)), agent_ids_gen[i%num_agent_pairs,0], first_intersecting_timesteps[0]]
            hyp_cross_point_agent2 = Y_hypothetical[int(np.floor(i/num_agent_pairs)), agent_ids_gen[i%num_agent_pairs,1], first_intersecting_timesteps[1]]

            true_Y_agent1 = Y[int(np.floor(i/num_agent_pairs)), agent_ids_gen[i%num_agent_pairs,0]]
            true_Y_agent2 = Y[int(np.floor(i/num_agent_pairs)), agent_ids_gen[i%num_agent_pairs,1]]

            dist_2_hyp_cross_point_agent1 = torch.linalg.norm(hyp_cross_point_agent1 - true_Y_agent1, axis=-1)
            dist_2_hyp_cross_point_agent2 = torch.linalg.norm(hyp_cross_point_agent2 - true_Y_agent2, axis=-1)

            closest_timestep_agent1 = torch.argmin(dist_2_hyp_cross_point_agent1)
            closest_timestep_agent2 = torch.argmin(dist_2_hyp_cross_point_agent2)

            if closest_timestep_agent1 < closest_timestep_agent2:
                crossing_class_gen[int(np.floor(i/num_agent_pairs)), i%num_agent_pairs] = 1

            elif closest_timestep_agent1 > closest_timestep_agent2:
                crossing_class_gen[int(np.floor(i/num_agent_pairs)), i%num_agent_pairs] = 2
        else:
            crossing_class_gen[int(np.floor(i/num_agent_pairs)), i%num_agent_pairs] = 0
    ...
    # remove nan entries
    existing_agents = ~torch.isnan(Y_hypothetical).any(axis=(2,3))
