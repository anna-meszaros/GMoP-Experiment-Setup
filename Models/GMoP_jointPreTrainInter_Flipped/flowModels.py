...
from GMoP_jointPreTrainInter_Flipped.spline_flow import NeuralSplineFlow
from GMoP_jointPreTrainInter_Flipped.SocialEncodingModule import SocialInterGNN
...
from GMoP_jointPreTrainInter_Flipped.dag_utils import dagification, kahn_toposort
from GMoP_jointPreTrainInter_Flipped.LaneGCN import M2A, MapNet, graph_gather, gpu, to_long
...
class GMoP(nn.Module):
    ...
    def __init__(self, pred_steps, alpha, beta, gamma, scene_encoder=None, B=15., 
        ...
        self.obs_encoder_inter = nn.ModuleDict({})
        ...
        for t in self.t_unique:
            ...
            self.obs_encoder_inter[t_key] = RNN(nin=2, nout=self.obs_encoding_size, es=self.es_rnn,
                                        hs=self.hs_rnn, nl=self.n_layers_rnn, device=device)
            self.obs_encoder_inter[t_key].to(device)
    ...
    def _encode_trajectories(self, x, T, interaction=False):
        ...
        for t in self.t_unique:
            ...
            if interaction: 
                x_enc[t_in], _ = self.obs_encoder_inter[t_key](x_in[t_in])
            else:
    ...
    def _encode_conditionals(self, x, T, scene=None, scene_graph=None):
        ...
        x_enc_inter = self._encode_trajectories(x, T, interaction=True)
        ...
        pair_info = torch.cat((x_enc_inter[:,None,:].repeat(1,max_num_agents,1,1), x_enc_inter[:,:,None,:].repeat(1,1,max_num_agents,1),
                T_one_hot[:,None,:].repeat(1,max_num_agents,1,1), T_one_hot[:,:,None].repeat(1,1,max_num_agents,1), 
                D, ang[:,:,:,None]), dim = -1)
        ...
        # Set upper triangular part of adjacency matrix to 1 for agents if interaction class argmax is index 2
        # adjacency_matrix[:,torch.triu(torch.ones(max_num_agents, max_num_agents), diagonal = 1) == 1] = (interaction_class.argmax(dim = -1) == 1).float()
        adjacency_matrix[:,torch.triu(torch.ones(max_num_agents, max_num_agents), diagonal = 1) == 1] = ((interaction_class.argmax(dim = -1) == 2) & 
                                                                                                            ((interaction_class.max(dim = -1))[0] > self.interaction_thresh)).float()
        edge_probs[:,torch.triu(torch.ones(max_num_agents, max_num_agents), diagonal = 1) == 1] = interaction_class[:, :, 2]

        # Set lower triangular part of adjacency matrix to 1 for agents if interaction class argmax is index 1
        ...
        adjacency_matrix[:,idx_sorted, idy_sorted] = ((interaction_class.argmax(dim = -1) == 1) & 
                                                        ((interaction_class.max(dim = -1))[0] > self.interaction_thresh)).float()
        edge_probs[:,idx_sorted, idy_sorted] = interaction_class[:, :, 1]
        ...
        if self.use_graph:
            ...
            x_centering_rel = x[:,0,[-1],:2] - x[:,0,[-2],:2]
