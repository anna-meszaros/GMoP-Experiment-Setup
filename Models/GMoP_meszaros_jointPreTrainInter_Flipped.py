...
import torch
import random
from GMoP_jointPreTrainInter_Flipped.flowModels import GMoP, Future_Encoder, Future_Decoder, Future_Seq2Seq, Scene_Encoder, InteractionClassifier
from GMoP_jointPreTrainInter_Flipped.LaneGCN import from_numpy

from GMoP_jointPreTrainInter_Flipped.multi_task_utils import get_crossing_trajectories, get_hypothetical_path_crossing

import pandas as pd
import pickle
import os

class GMoP_meszaros_jointPreTrainInter_Flipped(model_template):
    '''
    GMoP is a joint agent prediction model that combine factorized Normalizing Flows with
    ...
    Mészáros, A., Alonso-Mora, J., & Kober, J. (2025). Studying the Effect of Explicit Interaction Representations on Learning Scene-level 
    Distributions of Human Trajectories. arXiv preprint arXiv:2511.04375.
    '''
    def define_default_kwargs(self):
        ...
        if not ('es_gnn' in self.model_kwargs.keys()):
            self.model_kwargs['es_gnn'] = 64 #32
        ...
        if not('iCL_hdim' in self.model_kwargs.keys()):
            self.model_kwargs['iCL_hdim'] = 32 #128
        ...
        if not('use_img' in self.model_kwargs.keys()):
            self.model_kwargs['use_img'] = True

        if not('use_graph' in self.model_kwargs.keys()):
            self.model_kwargs['use_graph'] = True
        ...
        self.use_img = self.model_kwargs['use_img']
        self.use_graph = self.model_kwargs['use_graph']
    ...
    def setup_method(self):        
        ...
        self.max_t_O_train = self.num_timesteps_out #100
    ...
    def train_futureAE(self, T_all):
        ...
        # Use encoder of noninteractive GMoP model if available, as same training stuff is used
        ...
        if os.path.isfile(fut_model_file) and not self.model_overwrite:
            ...
            for file in model_files:
                ...
                # Check for GMoP model
                if model_name[:24] != 'GMoP_jointPreTrainInter_':
                    continue

                # Check for parameters
                model_kwargs_pot = model_name[24:].split('_')
            ...
            if eqivalent_file is not None and not self.model_overwrite:
                ...
                equiv_model_file = equiv_model_file[:-3] + '_AE'
                ...
                flow_dist_futMdl = GMoP(pred_steps=self.num_timesteps_out, alpha=self.alpha, beta=self.beta_noise, 
                ...
                for epoch in range(self.fut_ae_epochs):
                    ...
                    print('Train GMoP Autoencoder: Epoch ' + rjust_epoch + '/{}'.format(self.fut_ae_epochs), flush = True)
                    # Analyize memory:
                    ...
                    while not train_epoch_done:
                        ...
                        X, Y, T, _, _, _, _, Pred_agents, num_steps, Sample_id, _, train_epoch_done = self.provide_batch_data('train', self.batch_size, 
                                                                                            val_split_size = 0.1)
                    ...
                    with torch.no_grad():
                        ...
                        while not val_epoch_done:
                            X, Y, T, _, _, _, _, Pred_agents, num_steps, Sample_id, _, val_epoch_done = self.provide_batch_data('val', self.batch_size, 
                                                                                                    val_split_size = 0.1)
    ...
    def train_interactionClassifier(self, T_all):
        

        # Use encoder of noninteractive GMoP model if available, as same training stuff is used
        inter_classifier_file = self.model_file[:-4] + '_IC'
        if os.path.isfile(inter_classifier_file) and not self.model_overwrite:
            ...
            flow_dist_IC = GMoP(pred_steps=self.num_timesteps_out, alpha=self.alpha, beta=self.beta_noise, 
                                        gamma=self.gamma_noise, norm_rotation=True, device=self.device, 
                                        obs_encoding_size=obs_encoding_size, scene_encoding_size=scene_encoding_size, 
                                        n_layers_rnn=n_layers_rnn, es_rnn=hs_rnn, hs_rnn=hs_rnn, T_all=T_all)
            
            inter_classifier, flow_dist_IC.obs_encoder_inter = pickle.load(open(inter_classifier_file, 'rb'))
            print('Interaction Classifier loaded')
            ...
            inter_classifier = InteractionClassifier(pair_info_dim=2 * obs_encoding_size + 2 * len(t_unique) + 3, num_classes=3, iCL_hdim=self.iCL_hdim)

            module_list = list(inter_classifier.parameters())

            for t in t_unique:
                t_key = str(int(t.detach().cpu().numpy().astype(int)))
                module_list += list(flow_dist_IC.obs_encoder_inter[t_key].parameters())

            optim = torch.optim.AdamW(module_list, lr=self.flow_lr, weight_decay=self.flow_wd)
            lr_sc = torch.optim.lr_scheduler.ExponentialLR(optim, gamma = self.flow_lr_decay)
            ...
            for epoch in range(self.fut_ae_epochs):
                print('')
                rjust_epoch = str(epoch).rjust(len(str(self.fut_ae_epochs)))
                print('Train GMoP Interaction Classifier: Epoch ' + rjust_epoch + '/{}'.format(self.fut_ae_epochs), flush = True)
                # Analyize memory:
                inter_classifier.to(device = self.device)
                inter_classifier.train()
                train_loss = []
                val_losses_epoch = []
                ...
                while not train_epoch_done:
                    ...
                    X, Y, T, _, _, _, _, _, _, Sample_id, _, train_epoch_done = self.provide_batch_data('train', self.batch_size, 
                                                                                        val_split_size = 0.1)
                    ...
                    x_enc = flow_dist_IC._encode_trajectories(past_data, T, interaction=True)               
                    ...
                    ang = flow_dist_IC._get_agent_pair_angles(past_data, D, max_num_agents)
                    
                    # Get agent pair information
                    T_one_hot = (T.unsqueeze(-1) == flow_dist_IC.t_unique.unsqueeze(0)).float() # shape: num_samples x max_num_agents x num_classes
                    ...
                    if self.crossing_task:
                        ...
                        crossing_pair_ids = self.Crossing_Task_Data.loc[Sample_id, 'pair_ids'].values.tolist()
                        crossing_classes = self.Crossing_Task_Data.loc[Sample_id, 'classes'].values.tolist()
                        
                        # print(' ')
                        # print('crossing_pair_ids shape: ', np.concatenate(crossing_pair_ids).shape)
                        # print('crossing_pair_ids sum: ', np.nansum(np.concatenate(crossing_pair_ids)))
                        ...
                        # print('pair_info t1: ', pair_info_t1.isnan().sum())
                        # print('pair_info: ', pair_info.isnan().sum())
                        # print('X ', X.isnan().sum())
                        # print('pair_info t1 sum: ', pair_info_t1.nansum())
                        # print('pair_info sum: ', pair_info.nansum())
                        # print('X sum', X.nansum())
                        interaction_class = inter_classifier(pair_info_t1, task=1)
                        loss = CE_loss(interaction_class, classes_one_hot)

                    elif self.hyp_crossing_task:

                        hyp_crossing_pair_ids = self.Hyp_Crossing_Task_Data.loc[Sample_id, 'pair_ids'].values.tolist()
                        hyp_crossing_classes = self.Hyp_Crossing_Task_Data.loc[Sample_id, 'classes'].values.tolist()
                        ...
                        interaction_class = inter_classifier(pair_info_t2, task=2)
                        loss = CE_loss(interaction_class, classes_one_hot)
                ...
                inter_classifier.to(device = self.device)
                inter_classifier.eval()
                        
                with torch.no_grad():
                    
                    val_epoch_done = False
                    while not val_epoch_done:
                        X, Y, T, _, _, _, _, _, _, Sample_id, _, val_epoch_done = self.provide_batch_data('val', self.batch_size, 
                                                                                                val_split_size = 0.1)
                        ...
                        if self.crossing_task:
                            ...
                            pair_info_t1 = torch.concatenate(pair_info_t1)
                            interaction_class = inter_classifier(pair_info_t1, task=1)
                            val_loss = CE_loss(interaction_class, classes_one_hot)
                    ...
                    # Early stopping for interaction classifier
                    # Check for convergence
                    if epoch > 30:
                        best_val_step = np.argmin(val_losses)
            ...
            self.train_loss[0, :len(val_losses)] = np.array(val_losses)
            os.makedirs(os.path.dirname(inter_classifier_file), exist_ok=True)

            if self.crossing_task:
                inter_classifier.final_class0.weight = inter_classifier.final_class1.weight
            elif self.hyp_crossing_task:
                inter_classifier.final_class0.weight = inter_classifier.final_class2.weight

            # Save model    
            pickle.dump((inter_classifier, flow_dist_IC.obs_encoder_inter), open(inter_classifier_file, 'wb'))

        return inter_classifier, flow_dist_IC.obs_encoder_inter


    def train_flow(self, fut_model, interaction_classifier, obs_encoder_inter, T_all):
        use_map = self.can_use_map and self.has_map and self.use_img
        use_graph = self.can_use_graph and self.has_graph and self.use_graph
        ...
        flow_dist = GMoP(pred_steps=self.fut_enc_sz, alpha=self.alpha, beta=self.beta_noise, gamma=self.gamma_noise, 
        ...
        for param in interaction_classifier.parameters():
            param.requires_grad = False 
            param.grad = None
        ...
        for t in t_unique:
            t_key = str(int(t.detach().cpu().numpy().astype(int)))
            for param in obs_encoder_inter[t_key].parameters():
                param.requires_grad = False 
                param.grad = None
            flow_dist.obs_encoder_inter[t_key] = obs_encoder_inter[t_key]
        
            
        flow_dist.interaction_classifier = interaction_classifier
        
        flow_dist_file = self.model_file[:-4] + '_NF'
        flow_dist_pt_file = self.model_file[:-4] + '_NF.pt'

        epoch_file = self.model_file[:-4] + '_NF_epoch.pkl'
        val_losses_file = self.model_file[:-4] + '_NF_val_losses.pkl'
        val_losses = []
        ...
        if os.path.exists(epoch_file):
            ...
            self.flow_lr = self.flow_lr * (self.flow_lr_decay**start_epoch)

            val_losses = pickle.load(open(val_losses_file, 'rb'))
        ...
        if os.path.isfile(flow_dist_file) and not self.model_overwrite:
            ...
            for step in range(start_epoch, self.flow_epochs):
                ...
                while not train_epoch_done:
                    ...
                    X, Y, T, _, img, _, graph, _, _, Sample_id, _, train_epoch_done = self.provide_batch_data('train', self.batch_size, 
                                                                                           val_split_size = 0.1)
                    ...
                    if img is not None and self.use_img:
                        img_train = img.permute(0,1,4,2,3)
                    else:
                        img_train = None
                    ...
                    logprob, j_logprob, num_agents_per_sample = flow_dist.log_prob(out, past_data, T, scene=img_train, scene_graph=graph)
                ...
                with torch.no_grad():
                    val_epoch_done = False
                    while not val_epoch_done:
                        X, Y, T, _, img, _, graph, _, _, Sample_id, _, val_epoch_done = self.provide_batch_data('val', self.batch_size, 
                                                                                                val_split_size = 0.1)
                        X[(np.isnan(X).any((2,3)) | np.isnan(Y).any((2,3)))] = np.nan
                        T[(np.isnan(X).any((2,3)) | np.isnan(Y).any((2,3)))] = '0'
                        Y[(np.isnan(X).any((2,3)) | np.isnan(Y).any((2,3)))] = np.nan
                ...
                pickle.dump(val_losses, open(val_losses_file, 'wb'))
    ...
    def prep_task_data(self):

        task_data_file = self.splitter.split_file[:-4]

        Crossing_Task_Data = []
        Hyp_Crossing_Task_Data = []

        epoch_done = False
        batch_num = 0

        while not epoch_done:
            X, Y, T, _, _, _, _, _, _, Sample_id, _, epoch_done = self.provide_batch_data('train', 16, 
                                                                                    val_split_size = 0.0)
            ...
            crossing_pair_ids, crossing_classes = get_crossing_trajectories(Y=Y, 
                                                                            T=T,
                                                                            device=self.device)
            
            hyp_crossing_pair_ids, hyp_crossing_classes = get_hypothetical_path_crossing(X=X, 
                                                                                         Y=Y, 
                                                                                         T=T, 
                                                                                         dt=self.dt,
                                                                                         dataset=self.data_set.get_name()['print'],
                                                                                         device=self.device)
                                                                                         
            for i in range(X.shape[0]):
                ct_data = pd.Series(np.zeros(3, object), index = ['id', 'pair_ids', 'classes'])
                hct_data = pd.Series(np.zeros(3, object), index = ['id', 'pair_ids', 'classes'])

                ct_data['id'] = Sample_id[i]
                ct_data['pair_ids'] = crossing_pair_ids[i]
                ct_data['classes'] = crossing_classes[i]

                hct_data['id'] = Sample_id[i]
                hct_data['pair_ids'] = hyp_crossing_pair_ids[i]
                hct_data['classes'] = hyp_crossing_classes[i]

                Crossing_Task_Data.append(ct_data)
                Hyp_Crossing_Task_Data.append(hct_data)

            print('Batch {}'.format(batch_num))
            batch_num += 1

        Crossing_Task_Data = pd.DataFrame(Crossing_Task_Data)
        Hyp_Crossing_Task_Data = pd.DataFrame(Hyp_Crossing_Task_Data)

        Crossing_Task_Data = Crossing_Task_Data.sort_values('id')
        Hyp_Crossing_Task_Data = Hyp_Crossing_Task_Data.sort_values('id')

        Crossing_Task_Data = Crossing_Task_Data.set_index('id')
        Hyp_Crossing_Task_Data = Hyp_Crossing_Task_Data.set_index('id')

        if not os.path.exists(os.path.dirname(task_data_file)):
            os.makedirs(os.path.dirname(task_data_file))

        Crossing_Task_Data.to_pickle(task_data_file + '_ct_data.pkl')
        Hyp_Crossing_Task_Data.to_pickle(task_data_file + '_hct_data.pkl')
    ...
    def train_method(self):  
        ...
        if not os.path.isfile(task_data_file  + '_ct_data.pkl') or \
           not os.path.isfile(task_data_file  + '_hct_data.pkl'):
            
            self.prep_task_data()

            
        if self.crossing_task:
            self.Crossing_Task_Data = pd.read_pickle(task_data_file  + '_ct_data.pkl')

        if self.hyp_crossing_task:
            self.Hyp_Crossing_Task_Data = pd.read_pickle(task_data_file  + '_hct_data.pkl')
        ...
        self.interaction_classifier, self.obs_encoder_inter = self.train_interactionClassifier(T_all)
        
        self.flow_dist = self.train_flow(self.fut_model, self.interaction_classifier, self.obs_encoder_inter, T_all)
    ...
    def load_method(self):        
        ...
        self.interaction_classifier, self.obs_encoder_inter = pickle.load(open(self.model_file[:-4] + '_IC', 'rb'))
    ...
    def predict_method(self):
        ...
        while not prediction_done:
            X, T, _, img, _, graph, Pred_agents, num_steps, Sample_id, Agent_id, prediction_done = self.provide_batch_data('pred', self.batch_size)
            ...
            with torch.no_grad():
                ...
                if img is not None and self.use_img:
                    img_pred = img.permute(0,1,4,2,3)
                else:
                    img_pred = None
                ...
                samples_rel, log_probs, j_log_probs, batch_ids, row_ids = self.flow_dist.sample(self.num_samples_path_pred, X[..., :2], T, scene=img_pred, scene_graph=graph)
    ...
    def get_name(self = None):
        ...
        kwargs_str += 'seed' + str(self.model_kwargs['seed']) + '_' + \
                      ...
                      'interThresh' + str(np.round(self.model_kwargs['interaction_thresh'], 2)) 
        ...
        model_str = 'GMoP_jointPreTrainInter_Flip_' + kwargs_str 
