...
import torch
import random
from GMoP_multiTask.flowModels import GMoP, Future_Encoder, Future_Decoder, Future_Seq2Seq, Scene_Encoder
from GMoP_multiTask.LaneGCN import from_numpy

from GMoP_multiTask.multi_task_utils import get_crossing_trajectories, get_hypothetical_path_crossing

import pandas as pd
import pickle
import os

class GMoP_meszaros_multiTask(model_template):
    '''
    ...
    def train_futureAE(self, T_all):
        ...
        if os.path.isfile(fut_model_file) and not self.model_overwrite:
            ...
            for file in model_files:
                ...
                if model_name[:15] != 'GMoP_multiTask_':
                    continue

                # Check for parameters
                model_kwargs_pot = model_name[15:].split('_')
        ...
        if os.path.isfile(flow_dist_file) and not self.model_overwrite:
            ...
            for step in range(start_epoch, self.flow_epochs):
                ...
                while not train_epoch_done:
                    ...
                    if self.crossing_task or self.hyp_crossing_task:
                        ...
                        if self.crossing_task:
                            print(epoch_string + ' - Batch {} - run crossing task'.format(batch), flush = True)
                            ...
                            print(epoch_string + ' - Batch {} - post process crossing task'.format(batch), flush = True)
                            ...
                            print(epoch_string + ' - Batch {} - calculate crossing task loss'.format(batch), flush = True)
                        ...
                        if self.hyp_crossing_task:
                            print(epoch_string + ' - Batch {} - run hypothetical crossing task'.format(batch), flush = True)
                            ...
                            print(epoch_string + ' - Batch {} - post process hypothetical crossing task'.format(batch), flush = True)
                            ...
                            print(epoch_string + ' - Batch {} - calculate hypothetical crossing task loss'.format(batch), flush = True)
                        ...
                        loss_multi_task = (loss_task1 + loss_task2)*self.model_kwargs['multiTask_lossWeight']
    ...
    def get_name(self = None):
        ...
        model_str = 'GMoP_multiTask_' + kwargs_str 
