...
import torch
import random
from GMoP_multiTask_Euclidean.flowModels import GMoP, Future_Encoder, Future_Decoder, Future_Seq2Seq, Scene_Encoder
from GMoP_multiTask_Euclidean.LaneGCN import from_numpy

import pandas as pd
import pickle
import os

class GMoP_meszaros_multiTask_Euclidean(model_template):
    '''
    ...
    def train_futureAE(self, T_all):
        ...
        if os.path.isfile(fut_model_file) and not self.model_overwrite:
            ...
            for file in model_files:
                ...
                if model_name[:20] != 'GMoP_multiTask_Eucl_':
                    continue

                # Check for parameters
                model_kwargs_pot = model_name[20:].split('_')
    ...
    def train_flow(self, fut_model, T_all):
        ...
        flow_dist = GMoP(pred_steps=self.fut_enc_sz, alpha=self.alpha, beta=self.beta_noise, gamma=self.gamma_noise, 
                               ...
                               n_layers_gnn=1, es_gnn=self.es_gnn, T_all = T_all, lanegcn_configs=self.lanegcn_config,
        ...
        if os.path.isfile(flow_dist_file) and not self.model_overwrite:
            ...
            for step in range(start_epoch, self.flow_epochs):
                ...
                while not train_epoch_done:
                    ...
                    if dataset != 'Argoverse' and dataset != 'Interaction':
                        heading = torch.atan2(X_t0[..., 1], X_t0[..., 0])
                        X_in = torch.cat([past_data, torch.tile(heading[..., None, None], (1, 1, past_data.shape[2], 1))], axis=-1)
                    else:
                        X_in = torch.cat([past_data, X[..., :, -1:]], axis=-1)
                        
                    logprob, j_logprob, num_agents_per_sample = flow_dist.log_prob(out, X_in, T, scene=img_train, scene_graph=graph)
                ...
                with torch.no_grad():
                    ...
                    while not val_epoch_done:
                        ...
                        if dataset != 'Argoverse' and dataset != 'Interaction':
                            heading = torch.atan2(X_t0[..., 1], X_t0[..., 0])
                            X_in = torch.cat([past_data_val, torch.tile(heading[..., None, None], (1, 1, past_data_val.shape[2], 1))], axis=-1)
                        else:
                            X_in = torch.cat([past_data_val, X[..., :, -1:]], axis=-1)

                        log_prob, j_logprob, num_agents_per_sample = flow_dist.log_prob(out, X_in, T, scene=img_val, scene_graph=graph)
    ...
    def predict_method(self):
        ...
        while not prediction_done:
            ...
            with torch.no_grad():
                ...
                if dataset != 'Argoverse' and dataset != 'Interaction':
                    heading = torch.atan2(X_t0[..., 1], X_t0[..., 0])
                    X_in = torch.cat([X[..., :2], torch.tile(heading[..., None, None], (1, 1, X[..., :2].shape[2], 1))], axis=-1)
                else:
                    X_in = torch.cat([X[..., :2], X[..., :, -1:]], axis=-1)

                samples_rel, log_probs, j_log_probs, batch_ids, row_ids = self.flow_dist.sample(self.num_samples_path_pred, X_in, T, scene=img_pred, scene_graph=graph)
    ...
    def get_name(self = None):
        ...
        kwargs_str += 'seed' + str(self.model_kwargs['seed']) + '_' + \
                      ...
                      'iCL_hdim' + str(self.model_kwargs['iCL_hdim'])
        ...
        model_str = 'GMoP_multiTask_Eucl_' + kwargs_str 
