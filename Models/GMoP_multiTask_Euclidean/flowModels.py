...
from GMoP_multiTask_Euclidean.spline_flow import NeuralSplineFlow
from GMoP_multiTask_Euclidean.SocialEncodingModule import SocialInterGNN
...
from GMoP_multiTask_Euclidean.dag_utils import dagification, kahn_toposort
from GMoP_multiTask_Euclidean.LaneGCN import M2A, MapNet, graph_gather, gpu, to_long
...
class GMoP(nn.Module):
    ...
    def __init__(self, pred_steps, alpha, beta, gamma, scene_encoder=None, B=15., 
                ...
                n_layers_gnn=4, es_gnn=64, T_all = None, lanegcn_configs=None, iCL_hdim=128):
    ...
    def _encode_conditionals(self, x, T, scene=None, scene_graph=None):
        ...
        x_traj = x[..., :2]
        x_enc = self._encode_trajectories(x_traj, T)
        ...
        D = x_traj[:,None,:,-1] - x_traj[:,:,None,-1] # shape: num_samples x max_num_agents x max_num_agents x 2
        eucl_dist = torch.sqrt(torch.sum(D ** 2, dim=-1)) # shape: num_samples x max_num_agents x max_num_agents
        focal_ang = torch.atan2(D[:,:,:,1], D[:,:,:,0]) - x[...,None,-1,2]  # shape: num_samples x max_num_agents x max_num_agents

        # wrap angles to [-pi, pi]
        focal_ang[focal_ang > torch.pi] -= 2 * torch.pi 
        focal_ang[focal_ang < -torch.pi] += 2 * torch.pi

        ang = self._get_agent_pair_angles(x_traj, D, max_num_agents)
        ...
        dist_thresh = 100 # distance threshold for adjacency
        adjacency_mask = (eucl_dist <= dist_thresh) & (torch.abs(focal_ang.permute(0,2,1)) <= torch.pi/2)
        adjacency_mask[:, torch.arange(max_num_agents), torch.arange(max_num_agents)] = 0 # remove self loops

        bi_dir_edges = adjacency_mask & adjacency_mask.transpose(1,2)
        b_idx, i_idx, j_idx = torch.where(bi_dir_edges.cpu() & ~torch.tile(torch.eye(adjacency_mask.shape[1],dtype=bool)[None,:,:], (adjacency_mask.shape[0],1,1)))

        # Get angles from which agent i sees j and agent j sees i
        f_ang_ij = torch.abs(focal_ang[b_idx, i_idx, j_idx])
        f_ang_ji = torch.abs(focal_ang[b_idx, j_idx, i_idx])

        # Remove the edge from a bi-directional edge that has the larger angle; assumes that the agent with the smaller angle is more focused on the other agent
        remove_mask = (f_ang_ij < f_ang_ji).cpu() # flipped logic since if i sees j at a smaller angle, than j is affect i more than vice versa
        b_idx = b_idx[remove_mask]
        i_idx = i_idx[remove_mask]
        j_idx = j_idx[remove_mask]

        # Remove the edges from the adjacency matrix
        adjacency_mask[b_idx, i_idx, j_idx] = 0

        adjacency_matrix[adjacency_mask] = 1

        edge_probs_help = (dist_thresh - eucl_dist)/dist_thresh
        # edge_probs_help[edge_probs_help<0] = 0 # set negative values to 0
        edge_probs_help[:, torch.arange(max_num_agents), torch.arange(max_num_agents)] = 0
        edge_probs_help[adjacency_matrix==0] = 0
        edge_probs = edge_probs_help #torch.nansum(edge_probs_help, dim=-2, keepdim=True)#.permute(0,2,1)
        ...
        if (bi_dir_edges.cpu() & ~torch.tile(torch.eye(adjacency_mask.shape[1],dtype=bool)[None,:,:], (adjacency_mask.shape[0],1,1))).any():
            b_idx, i_idx, j_idx = torch.where(bi_dir_edges.cpu() & ~torch.tile(torch.eye(adjacency_mask.shape[1],dtype=bool)[None,:,:], (adjacency_mask.shape[0],1,1)))

            # Get number of neighbours influencing each agent
            num_neighbours = torch.nansum(adjacency_mask, dim=-2) #check dimension
	
            # Remove the edge from a bi-directional edge that has the higher number of neighbours;
            # assumes that the agent with the smaller number of neighbours is more focused on the other agent

            remove_mask = (num_neighbours[b_idx, i_idx] < num_neighbours[b_idx, j_idx]).cpu()
            b_idx = b_idx[remove_mask]
            i_idx = i_idx[remove_mask]
            j_idx = j_idx[remove_mask]

            adjacency_mask = torch.zeros_like(adjacency_matrix, dtype=torch.bool, device=self.device)

            # Remove the edges from the adjacency matrix
            adjacency_mask[b_idx, i_idx, j_idx] = 1

            adjacency_matrix[adjacency_mask] = 0
        ...
        if (bi_dir_edges.cpu() & ~torch.tile(torch.eye(adjacency_mask.shape[1],dtype=bool)[None,:,:], (adjacency_mask.shape[0],1,1))).any():
            upper_triangle = torch.broadcast_to(torch.triu(torch.ones(adjacency_mask.shape[1], adjacency_mask.shape[1])), (adjacency_mask.shape[0], adjacency_mask.shape[1], adjacency_mask.shape[1])).bool()
            b_idx, i_idx, j_idx = torch.where(bi_dir_edges.cpu() & upper_triangle & ~torch.tile(torch.eye(adjacency_mask.shape[1],dtype=bool)[None,:,:], (adjacency_mask.shape[0],1,1)))

            # Randomly remove one of the edges from a bi-directional edge
            # group the indices belonging to a bidirectional edge
            grouped_indices = torch.stack([torch.stack([b_idx, i_idx, j_idx], dim=1), torch.stack([b_idx, j_idx, i_idx], dim=1)], dim=1)
            # grouped_indices = grouped_indices.view(-1, 2, 3) # shuffle and group in pairs

            filter_choice = np.random.rand(int(len(i_idx))) > 0.5
            discard_choice = torch.stack((torch.tensor(~filter_choice), torch.tensor(filter_choice)), axis=1)
  
            print("Final filtering applied on " + str(int(len(i_idx))) + " edges")

            discard_ids = grouped_indices[discard_choice]

            discard_b_idx, discard_i_idx, discard_j_idx = discard_ids[:,0], discard_ids[:,1], discard_ids[:,2]

            adjacency_mask = torch.zeros_like(adjacency_matrix, dtype=torch.bool, device=self.device)

            adjacency_mask[discard_b_idx, discard_i_idx, discard_j_idx] = 1
            # Remove the edges from the adjacency matrix
        ...
        if self.use_graph:
            ...
            x_centering_rel = x_traj[:,0,[-1],:2] - x_traj[:,0,[-2],:2]
            rot_angles_rad = -1 * torch.arctan2(x_centering_rel[:,0,1], x_centering_rel[:,0,0])
            cos_angles = torch.cos(rot_angles_rad)
            sin_angles = torch.sin(rot_angles_rad)
            ...
            rot_matrices_expanded = rotation_matrices.view(rotation_matrices.shape[0], 1, 1, 2, 2)

            x_centered = x_traj-x_traj[:,0,-1].unsqueeze(1).unsqueeze(1)

            x_centered = torch.matmul(rot_matrices_expanded, x_centered[..., :2].unsqueeze(-1)).squeeze(-1)
