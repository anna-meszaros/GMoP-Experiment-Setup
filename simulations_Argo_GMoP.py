...
Data_params = [{'dt': 0.1, 'num_timesteps_in': (50, 50), 'num_timesteps_out': (60, 60)}] 
...
Metrics = [{'metric': 'minADE_joint', 'kwargs': {'num_preds': 20}}, {'metric': 'minFDE_joint', 'kwargs': {'num_preds': 20}}, 'KDE_NLL_joint']
 
print('Correlation between metrics')
corr = np.corrcoef(R, rowvar=False)
print(corr)
M_std = M.copy()
M['minADE20_joint'] = R[:, 0]
M['minFDE20_joint'] = R[:, 1]
M['KDE_NLL_joint'] = R[:, 2]
 
M.to_excel("Argo_Results.xlsx")

M_std['minADE20_joint'] = Std[:, 0]
M_std['minFDE20_joint'] = Std[:, 1]
M_std['KDE_NLL_joint'] = Std[:, 2]

M_std.to_excel("Argo_Results_std.xlsx")